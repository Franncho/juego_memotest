...
def crear_tablero():
    ...
    tablero["tiempo_ultimo_destape"] = pygame.time.get_ticks(0)
...
def generar_lista_tarjetas() -> list:
    ...
    random.shuffle(lista_id)  # Desordeno los identificadores de forma aleatoria
    ...
    for x in range(0, CANTIDAD_TARJETAS_H * ANCHO_TARJETA, ANCHO_TARJETA):
        for y in range(0, CANTIDAD_TARJETAS_V * ALTO_TARJETA, ALTO_TARJETA):
            identificador = lista_id
            nombre_imagen = f"{identificador}.png"
            nombre_imagen_escondida = "00.png"
            tarjeta_nueva = tarjeta.crear_tarjeta(nombre_imagen, identificador, nombre_imagen_escondida, x, y)
